...
const NewItemCard = ({ type, onClick, mousePos }) => {
  ...
  return (
    <div ref={cardRef} className="relative group cursor-pointer transition-all duration-300 hover:scale-[1.02]">
      ...
      <div
        onClick={onClick}
        className="relative w-full h-[220px] rounded-[2rem] overflow-hidden flex flex-col items-center justify-center
      ...
      >
        ...
        <div className="relative z-10 flex flex-col items-center justify-center">
          <div className="w-12 h-12 rounded-xl bg-gray-50 dark:bg-white/5 border border-gray-200/60 dark:border-white/10 flex items-center justify-center shadow-inner mb-4 group-hover:bg-gray-100 dark:group-hover:bg-white/10 transition-colors">
            {type === 'folder' ? <FiFolder size={24} className="text-gray-400 dark:text-gray-400" /> : <FiFileText size={24} className="text-gray-400 dark:text-gray-400" />}
          </div>
          <span className="font-medium text-lg text-gray-500 dark:text-gray-400 group-hover:text-gray-700 dark:group-hover:text-white transition-colors">New {type === 'folder' ? 'Folder' : 'Note'}</span>
        </div>
      </div>
    </div>
  );
